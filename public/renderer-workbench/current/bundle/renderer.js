(function () {
      ...
      const stylePolicy = documentPayload.stylePolicy === "preview" ? "preview" : "first-only";
      ...
      function selectedKeypointStyle(scenarioStyleKey, requestedStyle, occurrenceOrdinal) {
        return selectedCellStyle("KeyPointsCell", scenarioStyleKey, "default", undefined, requestedStyle, occurrenceOrdinal);
      }
      ...
      function renderGridShell(scenario, isNarrow, scenarioIndex) {
        ...
        slots.forEach(function (slot) {
          ...
          card.innerHTML = renderCell(slot, hasIncomingConnection, !!hasOutgoingConnection, relationshipStyle, cellStyle);
      ...
      function renderCell(slot, hasIncomingConnection, hasOutgoingConnection, relationshipStyle, cellStyle) {
        const cellType = safeText(slot.cell_type || slot.type);
        const content = slot.content || {};
        const props = slot.props || {};
        ...
        switch (cellType) {
          ...
          case "TripletCell":
            return renderTripletCell(content, props, slot, relationshipStyle);
          case "ImageCell":
      ...
      function renderTripletCell(content, props, slot, relationshipStyle) {
        ...
        const relation = relationshipClassToken(firstNonEmpty([props.relation, "cause_mechanism_effect"]));
        const orientation = resolveTripletOrientation(props, slot, content, relation);
        const tone = relationshipClassToken(firstNonEmpty([props.tone, "neutral"]));
        ...
        const renderMode = tripletRenderMode(relation, orientation);

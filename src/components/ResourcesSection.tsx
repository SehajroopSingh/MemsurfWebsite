...
export default function ResourcesSection() {
  return (
    <div className="mt-24 pt-16 border-t border-app-border">
        ...
        {resources.map((item, idx) => (
          ...
          >
            <div className="h-full overflow-hidden rounded-[24px] border border-app-border bg-app-surface p-3 transition-colors hover:bg-app-surfaceElevated">
              ...
              <div className="relative w-full aspect-[16/10] overflow-hidden rounded-2xl mb-4 bg-app-surfaceElevated flex items-center justify-center border border-app-border">
                <div className="transform transition-transform duration-500 group-hover:scale-110">
                  {item.type === 'Article' && <ArticleIcon className="w-16 h-16 text-app-textMuted relative z-10" />}
                  {item.type === 'Book' && <BookIcon className="w-16 h-16 text-app-textMuted relative z-10" />}
                  {item.type === 'Podcast' && <PodcastIcon className="w-16 h-16 text-app-textMuted relative z-10" />}
                </div>
              </div>
              
              {/* Content matching BlogCard text layout */}
              <div className="px-1 pb-1">
                <h3 className="text-lg font-semibold text-app-text mb-2 transition-colors line-clamp-3">
                  {item.title} <ExternalLinkIcon />
                </h3>
                <p className="text-sm text-app-textMuted font-medium">
      ...
      <div className="mt-16 bg-app-surface rounded-[28px] p-6 border border-app-border sm:p-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex-shrink-0 w-full md:w-auto mt-4 md:mt-0">
            <a href="mailto:contact@memsurf.com" className="block text-center bg-[var(--app-action)] text-[var(--app-action-text)] font-semibold px-8 py-3 rounded-full transition hover:opacity-[0.86]">
              Send it our way
            </a>
          </div>
        </div>

...
export default function StoreBadges({
  ...
  return (
    <div className="w-full sm:w-auto flex flex-col gap-3 items-center">
      <div className={`w-full sm:w-auto flex gap-3 sm:gap-4 sm:gap-6 justify-center items-center ${
        usesYouLearnStyle ? 'flex-col sm:flex-row' : 'flex-row'
      }`}>
        <a
          ...
          className={`${linkClassName} ${usesYouLearnStyle ? 'w-full max-w-[16rem] rounded-2xl bg-white p-1.5 shadow-sm sm:w-auto' : 'flex-1 sm:flex-none sm:w-auto'}`}
          aria-label="Download on the App Store"
        >
          <img
            src="/Download_on_App_Store/Black_lockup/SVG/Download_on_the_App_Store_Badge_US-UK_RGB_blk_092917.svg"
            alt="Download on the App Store"
            className={`w-full h-[58px] sm:h-[50px] md:h-[60px] sm:w-auto object-contain ${usesYouLearnStyle ? '' : 'invert'}`}
          />
        </a>
        {!hideGooglePlay && (
        <a
        ...
        >
          <img
            ...
            className="w-full h-[58px] sm:h-[50px] md:h-[60px] sm:w-auto object-contain"
          />
        </a>
        )}
      </div>
      ...
      <div className="text-xs sm:text-sm text-app-textMuted/90 flex flex-wrap justify-center items-center gap-x-3 gap-y-1">
        <a href="/privacy-policy" className="hover:text-app-text transition-colors">
          Privacy Policy
        </a>
        <span aria-hidden="true">•</span>
        <a href="/terms-of-service" className="hover:text-app-text transition-colors">
          Terms of Service
        </a>
        <span aria-hidden="true">•</span>
        <a
